...
CustomMouseArea {
    ...
    function inTopPanel(panel: Item, x: real, y: real): bool {
        ...
        if (panelHeight > 0) {
            return y <= (panelHeight + Config.border.thickness) && withinPanelWidth(panel, x, y);
        }
        const panelCenterX = bar.implicitWidth + panel.x + panel.width / 2;
        return y <= 8 && Math.abs(x - panelCenterX) <= 250;
    }

    function inUtilitiesPanel(panel: Item, x: real, y: real): bool {
        const panelHeight = panel.height * (1 - (panel.offsetScale ?? 0)); // qmllint disable missing-property
        if (panelHeight > 0) {
            return y <= (panelHeight + Config.border.thickness) && x >= (width - panel.width - Config.border.thickness);
        }
        return y <= 12 && x >= (width - 150);
    }
    ...
    onPositionChanged: event => {
        ...
        // Show launcher on hover only if enabled
