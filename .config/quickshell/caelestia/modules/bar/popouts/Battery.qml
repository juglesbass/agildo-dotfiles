...
Column {
    ...
    StyledText {
        text: UPower.displayDevice.isLaptopBattery ? `Restante: ${Math.round(UPower.displayDevice.percentage * 100)}%` : "Nenhuma bateria detectada"
    }
    ...
    StyledText {
        function formatSeconds(s: int, fallback: string): string {
            ...
            let comps = [];
            if (day > 0)
                comps.push(`${day} dia${day > 1 ? "s" : ""}`);
            if (hr > 0)
                comps.push(`${hr} hora${hr > 1 ? "s" : ""}`);
            if (min > 0)
                comps.push(`${min} min`);
        ...
        text: UPower.displayDevice.isLaptopBattery ? `Tempo ${UPower.onBattery ? "restante" : "até carregar"}: ${UPower.onBattery ? formatSeconds(UPower.displayDevice.timeToEmpty, "Calculando...") : formatSeconds(UPower.displayDevice.timeToFull, "Carga completa!")}` : `Perfil de energia: ${PowerProfile.toString(PowerProfiles.profile)}`
    }
