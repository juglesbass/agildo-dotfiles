...
ItemList {
    ...
    placeholderText: Nmcli.wifiEnabled ? qsTr("Nenhuma rede encontrada") : qsTr("Wi-Fi desativado")
    ...
    delegate: StateLayer {
        ...
        RowLayout {
            ...
            ColumnLayout {
                ...
                StyledText {
                    Layout.fillWidth: true
                    text: qsTr("Segurança: %1%2").arg(network.modelData.security).arg(network.modelData.active ? qsTr(" • Conectado") : Nmcli.hasSavedProfile(network.modelData.ssid) ? qsTr(" • Salvo") : "")
