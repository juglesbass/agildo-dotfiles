...
PageBase {
    ...
    ColumnLayout {
        ...
        RowButton {
            ...
            icon: "expand_content"
            text: qsTr("Mostrar todas as redes (%1)").arg(Nmcli.networks.length)
        ...
        ItemList {
            ...
            placeholderText: qsTr("Nenhum provedor de VPN configurado")
                ...
                RowLayout {
                    ...
                    ColumnLayout {
                        ...
                        StyledText {
                            ...
                            text: {
                                if (!provider.isSelected)
                                    return qsTr("Toque para selecionar");
                                if (VPN.connecting)
                                    return qsTr("Conectando...");
                                if (VPN.disconnecting)
                                    return qsTr("Desconectando...");
                                switch (VPN.status.state) {
                                case "connected":
                                    return qsTr("Conectado");
                                case "needs-auth":
                                    return VPN.status.reason || qsTr("Autenticação necessária");
                                case "error":
                                    return VPN.status.reason || qsTr("Ocorreu um erro");
                                default:
                                    return qsTr("Selecionado");
                                }
                            }
                            color: {
                                if (!provider.isSelected)
